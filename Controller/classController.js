...
const checkChildrenArray = async (request, result, next) => {
  let existing = await ChildSchema.find({
...
exports.getClass = (request, response, next) => {
  ClassSchema.findById({ _id: request.params.id })
    ...
    .then((data) => {
      if (data == null) {
        next(new Error("Class Is Not Found"));
      } else {
        response.status(200).json({ data });
      }
    })
...
exports.addClass = async (request, response, next) => {
  try {
    let supervisor = await TeacherSchema.findOne({
      _id: request.body.supervisor,
    });
    if (supervisor == null) {
      throw new Error("Teacher Is Not Found");
    }
    let notFound = await checkChildrenArray(request, response, next);
    if (notFound.length > 0) {
      throw new Error("Children [" + notFound + "] Not Found");
    }
...
exports.getClassChildren = (request, response, next) => {
  ClassSchema.findOne({ _id: request.params.id }, { children: 1 })
    ...
    .then((data) => {
      // console.log(data);
      if (data == null) {
        next(new Error("Can't Find Class"));
      } else {
        response.status(200).json({ data });
      }
    })
